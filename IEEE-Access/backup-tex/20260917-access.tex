...
\begin{tabular}{lcc}
\hline
\textbf{Component level}& \textbf{Median latency}& \textbf{Percentage of}\\
& \textbf{(p50)} & \textbf{total latency}\\
\hline
Backend Only (\textit{ghz}) & $\sim 2.8$ ms & 2.5\% \\
Full System (CPR, Before) & $\sim 400$ ms & 100\% \\
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=\columnwidth, height=0.4\textheight,keepaspectratio]{fig/base_vs_ctr_cpr.png} % or \linewidth
	\caption{Baseline vs controller performance evaluation of frontend apps both in Synchronous and Asyncronous framework gateway within Connection Per-Request (CPR) mode: (a) median and tail latency ($p50, p99$); (b) compute vs net and queue time latency ($t_{comp}, t_{net\_queue}$); (c) systems throughput; (d) resource utilization (CPU).}
	\label{base_vs_ctr_cpr}
\end{figure}

\begin{figure}[t!]
	\centering
	\includegraphics[width=\columnwidth, height=0.4\textheight,keepaspectratio]{fig/base_vs_ctr_pcr.png} % or \linewidth
	\caption{Baseline vs controller performance evaluation of frontend apps both in Synchronous and Asyncronous framework gateway within Persistent Connection Reuse (PCR) mode: (a) median and tail latency ($p50, p99$); (b) compute vs net and queue time latency ($t_{comp}, t_{net\_queue}$); (c) systems throughput; (d) resource utilization (CPU).}
	\label{base_vs_ctr_pcr}
\end{figure}
\end{comment}
...
\begin{table*}[htbp]
	\centering
	\caption{\textbf{Comparative End-to-End latency and controller suppression efficacy across load tiers (low - 200 RPS, moderate - 800 RPS, high - 1200 RPS).}}
...
\begin{table*}[htbp]
	...
	%\resizebox{\columnwidth}{!}{
		...
		\begin{tabular}{llllllll}
			\hline
			% This is the header sets
			\textbf{Architecture}& \textbf{Taget load}& \textbf{Degraded capacity}& \textbf{Projected crash}& \textbf{Actuation/Cutover}& \textbf{Controller CPU}& \textbf{Controller Mem}\\
			
			& \textbf{intervention threshold}& \textbf{at trigger (RPS)}& \textbf{point (RPS)}& \textbf{time (s)}& \textbf{overhead (m)}& \textbf{overhead (Mi)}\\
