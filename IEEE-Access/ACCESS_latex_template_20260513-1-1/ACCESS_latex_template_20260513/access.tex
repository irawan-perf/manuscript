...
\begin{figure}[t!]
	\centering
	\includegraphics[width=\columnwidth, height=0.4\textheight,keepaspectratio]{fig/baseline_latency.png} % or \linewidth
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=\columnwidth, height=0.4\textheight,keepaspectratio]{fig/baseline_backend.png} % or \linewidth
	\caption{Baseline performance evaluation of backend service that evaluate directly to the backend server using ghz tool with gRPC load request: (a) median and tail latency ($p50, p99$); (b) compute vs net and queue time latency ($t_{comp}, t_{net\_queue}$); (c) systems throughput; (d) resource utilization (CPU and Memory).}
...
\begin{figure*}[t!]
	...
	\begin{minipage}{1\textwidth}
		\centering
		\vspace*{0pt}
		\includegraphics[scale=0.4]{fig/async-controller-perf.png}
		%\vspace*{4pt}
		%\centering{\small(a) Synchronous CPR/PCR (Flask/Gunicorn).}
		%\label{eval-stg}
	\end{minipage}
	%\vspace{10pt}
	\caption{\textbf{Autonomic controller performance evaluation in asynchronous mode (FastAPI/Uvicorn): (a) Calculated saturation point ($\lambda_{sat}$); (b) service time ($S$); (c) Actuation time when envoy deployed to frontend pods; (d) controller cpu utilization; (e) controller memory utilization; (f) controllers throughput.}}
	\label{eval-ctr-async}
\end{figure*}
...
\begin{figure*}[t!]
	...
	\begin{minipage}{1\textwidth}
		\centering
		\vspace*{0pt}
		\includegraphics[scale=0.4]{fig/sync-controller-perf.png}
		%\vspace*{4pt}
		%\centering{\small(a) Synchronous CPR/PCR (Flask/Gunicorn).}
		%\label{eval-stg}
	\end{minipage}
	%\vspace{10pt}
	\caption{\textbf{Autonomic controller performance evaluation in synchronous mode (Flask/Gunicorn): (a) Calculated saturation point ($\lambda_{sat}$); (b) service time ($S$); (c) Actuation time when envoy deployed to frontend pods; (d) controller cpu utilization; (e) controller memory utilization; (f) controllers throughput.}}
	\label{eval-ctr-sync}
\end{figure*}
